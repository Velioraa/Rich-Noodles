...
// **Cek apakah nama produk sudah ada di database, kecuali produk yang sedang diupdate**
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $nama_produk = trim($_POST['nama_produk']);
    $fid_kategori = intval($_POST['fid_kategori']);
    ...
    // Cek apakah nama produk sudah ada di database, kecuali produk yang sedang diupdate
    $cek_produk = "SELECT id_produk FROM produk WHERE nama_produk = ? AND id_produk != ?";
    $stmt_cek = $conn->prepare($cek_produk);
    $stmt_cek->bind_param("si", $nama_produk, $id_produk);
    $stmt_cek->execute();
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($_FILES["gambar"]["name"])) {
        ...
        if (in_array($fileType, $allowTypes)) {
            if (move_uploaded_file($_FILES["gambar"]["tmp_name"], $target_file)) {
                $gambar_baru = "../asset/" . $gambar_nama;

                // Hapus gambar lama jika ada (kecuali default)
                if ($produk['gambar'] && file_exists("../" . $produk['gambar']) && $produk['gambar'] != "asset/default.png") {
            ...
            echo "<script>alert('Format gambar tidak valid! Hanya JPG, JPEG, PNG diperbolehkan.');</script>";
            exit();
        }
    }

   // Update data di database termasuk kode produk
   ...
   tanggal_expired = ?, 
   gambar = ?, 
   kode_barcode = ?  -- Menambahkan kode produk ke query
   WHERE id_produk = ?";

$stmt_update = $conn->prepare($sql_update);
$stmt_update->bind_param("sissdssssi", 
    $nama_produk, $fid_kategori, $brand, $modal, $harga_jual, 
    $deskripsi, $tanggal_expired, $gambar_baru, $kode_produk, $id_produk
);
...
<head>
    ...
    <title>Update Produk - Rich Noodles</title>
    ...
    <div class="container">
        ...
        <div class="form-section">
            <div class="current-product">
                <p>📦 Produk Saat Ini: <strong><?= htmlspecialchars($produk['nama_produk']) ?></strong></p>
            </div>

            <div class="profit-preview" id="profitPreview">
                <p>💰 Keuntungan: <span id="profitValue">Rp <?= number_format($produk['harga_jual'] - $produk['modal'], 0, ',', '.') ?></span></p>
            </div>

            <div class="barcode-info">
                <p>📋 Kode Barcode: <strong><?= htmlspecialchars($produk['kode_barcode']) ?></strong></p>
            </div>
            ...
            <form action="" method="POST" enctype="multipart/form-data">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label class="form-label" for="fid_kategori">Kategori</label>
                        <select class="form-control" id="fid_kategori" name="fid_kategori" required>
                            ...
                            while ($row = $result->fetch_assoc()) {
                                $selected = ($row['id_kategori'] == $produk['fid_kategori']) ? "selected" : "";
                                echo "<option value='" . $row['id_kategori'] . "' $selected>" . htmlspecialchars($row['kategori']) . "</option>";
                            }
                            ?>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="brand" name="brand" required>
                            <option value="Indomie" <?= ($produk['brand'] == "Indomie") ? "selected" : "" ?>>Indomie</option>
                            <option value="Samyang" <?= ($produk['brand'] == "Samyang") ? "selected" : "" ?>>Samyang</option>
                        </select>
                        <div class="form-text">Pilih brand produk</div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-control" id="modal" name="modal" 
                               value="<?= $produk['modal'] ?>" 
                               placeholder="Harga modal" min="0" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="date" class="form-control" id="tanggal_expired" name="tanggal_expired" 
                               value="<?= $produk['tanggal_expired'] ?>" required>
                    ...
                    <div class="form-group full-width">
                        ...
                        <div class="form-text">Format: JPG, JPEG, PNG (opsional - kosongkan jika tidak ingin mengubah)</div>
                    </div>

                    <div class="form-group full-width">
                        <label class="form-label" for="deskripsi">Deskripsi Produk</label>
                        <textarea class="form-control" id="deskripsi" name="deskripsi" 
                                  placeholder="Masukkan deskripsi produk" required><?= htmlspecialchars($produk['deskripsi']) ?></textarea>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            form.addEventListener('submit', function(e) {
                ...
                const kategori = document.getElementById('fid_kategori');
